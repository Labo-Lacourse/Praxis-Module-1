...
proc means data=regmulti n min max mean std;
var jbs170 scs134 pvs501;
run;
...
ods graphics on;
proc reg data=regmulti_filtre;
model jbs170=pvs501 scs134 /details stb;
...
data regmultiresid_outliers;
set regmultiresid;
if -3<zresid>3;
run;
proc freq data=regmultiresid_outliers;
tables state;
run;
